"""Steca-only curtailment-test: roept steca_adapter direct aan, zonder prijslogica.

Veiligste test om te verifieren dat menunavigatie + DOWN/UP-knoppen op de Steca
het juiste vermogenslimiet-resultaat geven. Geen prijscheck, geen state_store,
geen Kostal. Vraagt na elke fase om visuele bevestiging vanaf het LCD.

Gebruik:
    python3 tools/test_steca_curtail.py            # volledige cyclus: limited -> normal
    python3 tools/test_steca_curtail.py --only limited
    python3 tools/test_steca_curtail.py --only normal
"""
...
import steca_adapter  # noqa: E402
...
def _check_calibration(screens: dict) -> bool:
    """Controleert of er voldoende steca-hashes zijn om de adapter te draaien."""
    steca_keys = [k for k in screens if k.startswith("steca.")]
    required = {
        "steca.home",
        "steca.instellingen",
        "steca.service_item",
    ...
    print(f"  Steca-hashes in screens.json: {len(steca_keys)}/{len(required)}")
...
def _run_phase(label: str, desired_state: str, inv_cfg: dict, actions: dict, screens: dict) -> bool:
    ...
    try:
        steca_adapter.apply(desired_state, inv_cfg["ip"], inv_cfg, actions, screens)
    ...
    return _ask(f"Toont het Steca-display nu: {verwacht}?")
...
def main() -> None:
    ...
    if "steca" not in inverters:
        print("  FOUT: 'steca' staat niet in inverters.yaml.")
        sys.exit(1)
